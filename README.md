...
`/api/cron`は既存の監視処理を呼び出し、成功時は`ok: true`、失敗時は`ok: false`のJSONを返します。レスポンスにはMarket Guardの`marketOpen`、`rateFresh`、`canEvaluatePriceAlerts`、`canEvaluateMovementAlerts`、`reason`も含めます。

本番ではVercelの環境変数に`CRON_SECRET`を設定してください。`CRON_SECRET`が設定されている場合、`/api/cron`は以下のAuthorizationヘッダーが一致するリクエストだけを実行します。

```text
Authorization: Bearer <CRON_SECRET>
```

`CRON_SECRET`が未設定の場合は、ローカル開発用として認証なしで実行できます。`.env.local`やVercelの環境変数に設定する実値はGitHubへコミットしないでください。
...
Frankfurter利用時は日次レートのため`rateFresh`が`false`になる場合がありますが、市場時間内であれば買い通知価格・売り通知価格・接近通知幅の判定は継続します。一方、短時間変動アラートはFrankfurterでは無効化し、`movementSkipReason`は`unsupported_movement_provider`になります。リアルタイム系プロバイダーでは従来どおり、古いレートは価格通知・短時間変動通知ともに停止します。
...
`monitor_logs`にはCronまたは手動監視1回ごとの実行時刻、Market Guard状態、取得レート、判定結果、LINE通知実行結果を保存します。これにより、通知が出なかった場合も「市場停止」「古いレート」「クールダウン」「条件未達」などの流れを追跡できます。

短時間変動アラートは、現在レートと同じ`source`で、比較時間の前後5分にある`rate_logs`だけを比較対象にします。たとえば比較時間が15分の場合、10〜20分前の同一sourceログだけを使います。該当ログがない場合、sourceが異なる場合、レートが古い場合、または`RATE_API_PROVIDER=frankfurter`の場合はmovement判定を行わず、`monitor_logs.movement_skip_reason`に理由を保存します。

既存DBに追加する場合は、Supabase SQL Editorで`supabase/schema.sql`内の`monitor_logs`作成SQLを実行してください。
